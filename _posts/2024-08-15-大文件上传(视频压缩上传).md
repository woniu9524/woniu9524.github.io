...
const retryOperation = async (operation: () => Promise<any>, retries = MAX_RETRIES) => {
  try {
    ...
    if (retries > 0 && axios.isAxiosError(error) && error.response?.status === 502) {
  ...
  if (file.status === 'ready') {
    ...
    try {
      ...
      for (let i = 0; i < chunksToUpload.length; i++) {
        const { start, end } = chunksToUpload[i];
        ...
        formData.append('file', blob, `${file.raw.name}.part${i}`);
...
public class FFmpegUtils {
    ...
    public static void mergeAndCompressVideo(Path chunkDir, Path mergedFile, Path compressedFile, Integer testDuration) throws IOException, InterruptedException {
    ...
    private static void mergeChunks(Path chunkDir, Path mergedFile) throws IOException {
        ...
        int bufferSize = 512 * 1024 * 1024; //500MB 缓冲区
    ...
    public static void compressVideo(String inputFilePath, String outputFilePath, Integer testDuration) throws IOException, InterruptedException {
        List<String> command = buildFFmpegCommand(inputFilePath, outputFilePath, testDuration);
        ...
        if (!process.waitFor(1, TimeUnit.HOURS)) {
            ...
            throw new InterruptedException("视频压缩超时");
        }
        ...
        if (process.exitValue() != 0) {
            throw new IOException("视频压缩失败，FFmpeg 返回错误代码：" + process.exitValue());
        }
    }

    private static List<String> buildFFmpegCommand(String inputFilePath, String outputFilePath, Integer testDuration) {
        ...
        if (testDuration != null && testDuration > 0) {
            command.add("-t");
            command.add(String.valueOf(testDuration));
        }
        command.add("-r");
        command.add("30");
        command.add("-c:v");
